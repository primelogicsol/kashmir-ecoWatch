...
// 2. Reset page on filter
code = code.replace(
  'const handleFilterChange = (newFilters: any) => {',
  'const handleFilterChange = (newFilters: any) => {\n    setCurrentPage(1);'
);

// 3. Slice paginated species
code = code.replace(
  'const filteredSpecies = species; // Apply filters here based on activeFilters',
  'const filteredSpecies = species; // Apply filters here based on activeFilters\n  const totalPages = Math.ceil(filteredSpecies.length / ITEMS_PER_PAGE);\n  const paginatedSpecies = filteredSpecies.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);'
);

// 4. Update map to use paginatedSpecies
code = code.replace(
  '{filteredSpecies.map((species, index) => (',
  '{paginatedSpecies.map((species, index) => ('
);

// 5. Add Pagination UI below the grid
const paginationUI = `          </div>
          {totalPages > 1 && (
                ...
                {Array.from({ length: Math.min(5, totalPages) }).map((_, idx) => {
                  ...
                  return (
                    <Button
                      ...
                      className={pageNum === currentPage ? \`bg-gradient-to-r \${color}\` : 'text-slate-400 hover:text-white'}
                    >
                      {pageNum}
                    </Button>
                  );
