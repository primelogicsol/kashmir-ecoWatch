...
export default function HazardIntelligenceReportsPage() {
  ...
  const allReports = fieldReportsRegistry.filter(r => 
    r.modules.some(m => m.toLowerCase().includes('risk & monitoring') || m.toLowerCase().includes('hazard intelligence')) ||
    r.title.toLowerCase().includes('hazard intelligence')
    ...
    <main className="min-h-screen bg-slate-950">
      <Heading
        title={<>
            ...
            <span className="block whitespace-nowrap leading-[1.12] overflow-visible bg-gradient-to-r from-red-500 to-orange-600 bg-clip-text text-transparent">Hazard Intelligence Field Reports</span>
          </>}
        subtitle="Avalanche exposure logs, landslide geotechnical assessments, GLOF monitoring, and flood vulnerability reports."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {allReports.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center">
                ...
                <p className="text-slate-500 mt-2">Check back later for new Hazard Intelligence reports.</p>
              </div>
            )}
          </div>
        </div>
