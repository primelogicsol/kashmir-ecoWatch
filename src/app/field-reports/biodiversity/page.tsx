...
export default function BiodiversityReportsPage() {
  ...
  const allReports = fieldReportsRegistry.filter(r => 
    r.modules.some(m => m.toLowerCase().includes('biodiversity') || m.toLowerCase().includes('biodiversity')) ||
    r.title.toLowerCase().includes('biodiversity')
    ...
    <main className="min-h-screen bg-slate-950">
      <Heading
        title={<>
            ...
            <span className="block whitespace-nowrap leading-[1.12] overflow-visible bg-gradient-to-r from-purple-500 to-violet-600 bg-clip-text text-transparent">Biodiversity Field Reports</span>
          </>}
        subtitle="Species population censuses, avian migration tracking, phenology records, and flora conservation surveys."
        icon={<PawPrint className="w-6 h-6 text-purple-400" />}
        breadcrumbs={[
          { label: 'Home', href: '/' },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {allReports.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center">
                ...
                <p className="text-slate-500 mt-2">Check back later for new Biodiversity reports.</p>
              </div>
            )}
          </div>
        </div>
